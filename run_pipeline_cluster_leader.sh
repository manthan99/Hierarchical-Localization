...
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 <data_root> <blk_root> <traj_name> <output_root>"
    exit 1
fi
...
rsync -aP  "${TRAJ_DIR}/trimmed_vrs/leader_trimmed.vrs" \
   "${WORK_TRAJ}/trimmed_vrs/leader_trimmed.vrs"

# 2. MPS SLAM data
echo "  mps_leader_trimmed_vrs/slam/"
mkdir -p "${WORK_TRAJ}/mps_leader_trimmed_vrs/slam"
rsync -aP  "${TRAJ_DIR}/mps_leader_trimmed_vrs/slam/closed_loop_trajectory.csv" \
   "${WORK_TRAJ}/mps_leader_trimmed_vrs/slam/"
rsync -aP  "${TRAJ_DIR}/mps_leader_trimmed_vrs/slam/online_calibration.jsonl" \
   "${WORK_TRAJ}/mps_leader_trimmed_vrs/slam/"
rsync -aP  "${TRAJ_DIR}/mps_leader_trimmed_vrs/slam/semidense_points.csv.gz" \
   "${WORK_TRAJ}/mps_leader_trimmed_vrs/slam/"

# 3. BLK images + yml + pcd
echo "  blk/"
mkdir -p "${WORK_TRAJ}/blk/images"
rsync -a  "${BLK_TRAJ_DIR}"/blk/images/*.jpg "${WORK_TRAJ}/blk/images/" 2>/dev/null || true
rsync -a  "${BLK_TRAJ_DIR}"/blk/images/*.yml "${WORK_TRAJ}/blk/images/" 2>/dev/null || true
# Copy BLK point cloud (pcd/ply at top level of blk/)
rsync -a  "${BLK_TRAJ_DIR}"/blk/*.pcd "${WORK_TRAJ}/blk/" 2>/dev/null || true
rsync -a  "${BLK_TRAJ_DIR}"/blk/*.ply "${WORK_TRAJ}/blk/" 2>/dev/null || true
...
"${HLOC_DIR}/run_full_pipeline_leader.sh" "${WORK}" "${TRAJ}" "${WORK}"
...
rsync -a   "${WORK_TRAJ}/aria_raw"           "${FINAL_OUT}/" 2>/dev/null || true
rsync -a   "${WORK_TRAJ}/aria_undistorted"   "${FINAL_OUT}/" 2>/dev/null || true
rsync -a   "${WORK_TRAJ}/alignment"          "${FINAL_OUT}/" 2>/dev/null || true
rsync -a   "${WORK_TRAJ}/semidense_points.ply"       "${FINAL_OUT}/" 2>/dev/null || true
rsync -a   "${WORK_TRAJ}/blk_scan_aria_leader.ply"   "${FINAL_OUT}/" 2>/dev/null || true
rsync -a   "${WORK_TRAJ}/blk_scan_aria_leader_coarse.ply"   "${FINAL_OUT}/" 2>/dev/null || true
